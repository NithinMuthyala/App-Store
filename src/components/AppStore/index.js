...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, userText: ''}
  ...
  render() {
    const {activeTab, userText} = this.state
    console.log(userText)
    const requiredData = appsList.filter(
      eachItem => eachItem.category === activeTab,
    )
    const filteredData = requiredData.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(userText.toLowerCase()),
    )
    return (
